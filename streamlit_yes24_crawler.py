...
def init_db():
    ...
    cursor = conn.cursor()
    cursor.execute('CREATE TABLE IF NOT EXISTS seen_books (goods_no TEXT PRIMARY KEY)')
    conn.commit()
    return conn

def check_and_save_book(conn, goods_no):
    cursor = conn.cursor()
    cursor.execute('SELECT goods_no FROM seen_books WHERE goods_no = ?', (goods_no,))
    data = cursor.fetchone()

    if data:
        return False
    else:
        cursor.execute('INSERT INTO seen_books (goods_no) VALUES (?)', (goods_no,))
        conn.commit()
        return True
...
# 크롤링 함수
# ==========================================
def crawl_yes24(urls, conn, progress_bar=None):
    headers = {
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    is_new = check_and_save_book(conn, goods_no)
...
def main():
    ...
    if st.sidebar.button("🔄 크롤링 시작", type="primary", use_container_width=True):
        ...
        with st.spinner("데이터 수집 중..."):
            books = crawl_yes24(categories[selected_category], conn, progress_bar)
